...
test('une description en français devient un diagramme après validation', async () => {
  test.skip(!fs.existsSync(JAR), 'plantuml.jar absent : le rendu est impossible.');

  await activerFormalisme();
  await window.locator('button', { hasText: /^Analyser un texte$/ }).click();

  const analyse = window.locator('.dialog.analyse');
  await expect(analyse).toBeVisible();

  await analyse.locator('.analyse-texte').fill(
    [
      'Plateforme de réservation',
      'Le visiteur peut consulter le catalogue et créer un compte.',
      ...
      'Ensuite, il pourra le faire depuis son téléphone.',
    ].join('\n')
  );

  // Ce qui a été compris, et d'où cela vient : la proposition est vérifiable.
  const acteurs = analyse.locator('.analyse-groupe', { hasText: 'Acteurs' }).first();
  await expect(acteurs.locator('li')).toContainText(['Service de paiement']);

  const cas = analyse.locator('.analyse-groupe', { hasText: "Cas d'utilisation par acteur" });
  await expect(cas.locator('.analyse-libelle')).toContainText([
    'Visiteur : Consulter le catalogue',
    'Visiteur : Créer un compte',
    'Client : Réserver une prestation',
  ]);
  await expect(cas.locator('.analyse-source').first()).toContainText('ligne 2');

  // La phrase dont le sujet est un pronom reste visible, non interprétée.
  const ignorees = analyse.locator('.analyse-ignorees');
  await expect(ignorees.locator('li')).toHaveCount(1);
  await expect(ignorees).toContainText('depuis son téléphone');

  // Rien n'est écrit sans passer par le formulaire.
  await analyse.locator('button', { hasText: /^Ouvrir dans l'assistant$/ }).click();
  await expect(analyse).toHaveCount(0);

  const dialogue = window.locator('.dialog.assistant');
  await expect(dialogue).toBeVisible();
  await expect(dialogue.locator('.assistant-entete input')).toHaveValue(
    'Plateforme de réservation'
  );

  const lignes = dialogue.locator('.assistant-section', { hasText: 'Acteurs' }).first();
  await expect(lignes.locator('.assistant-ligne')).toHaveCount(3);
  await expect(lignes.locator('.assistant-ligne').nth(1).locator('input').first()).toHaveValue(
    'Client'
  );

  const apercu = dialogue.locator('.assistant-apercu pre');
  await expect(apercu).toContainText('Visiteur <|-[norank]- Client');
  await expect(apercu).toContainText('Reserver_une_prestation ..> S_authentifier : <<include>>');
